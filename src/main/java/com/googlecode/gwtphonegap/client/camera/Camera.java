...
public interface Camera {
   ...
   * 
   * opens the device's default camera application so that the user to take a photo. Once the photo
   * is taken, the camera application closes and your application is restored.
   * 
   * The image is base64 encoded and returned as a String in the cameraSuccess function. Since the
   * data is a String, you can do whatever you want with it, for example:
   * 
   * Render the image in an <img> tag (see example below) Save the data locally (LocalStorage,
